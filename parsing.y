...
%union {
    ...
    StmNode *stm;
	VarDecNode *vardec;
	std::vector<StmNode*> *stmlist;
    std::vector<ExpNode*> *explist;
    std::vector<VarDecNode*> *vardeclist;
}
%token<cVal> CHAR;
%token<iVal> INTEGER;
%token<sval> STRING;
...
;
var_decl:
identifier identifier{$$ = VarDecNode(*$1,*$2);}
|identifier identifier ASSIGN exp{$$ = VarDecNode{*$1,*$2,*$4}; }//?
|identifier identifier LBRACKET INTEGER RBRACKET {$$ = VarDecNode{*$1,*$2,$4};}
;
func_decl:
...
|exp SUB exp {$$ = new BinOpNode($2, *$1, *$3);}
